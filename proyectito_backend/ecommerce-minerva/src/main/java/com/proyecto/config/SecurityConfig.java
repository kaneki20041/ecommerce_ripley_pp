        ...
        http
            ...
            .authorizeHttpRequests(auth -> auth
                    ...
                    .requestMatchers(
                        ...
                        "/error",
                        "/actuator/health",
                        // Webhook de Mercado Pago: MP llama sin JWT (lo llama el servidor de MP)
                    ...
                    // 3. Dashboard del empleado (solo EMPLOYEE y ADMIN)
                    .requestMatchers(
                        "/api/employee/dashboard/**"
                    ).hasAnyRole("ADMIN", "EMPLOYEE")

                    // 4. Dashboard analítico del admin (solo ADMIN)
                    .requestMatchers(
                        "/api/admin/dashboard/**"
                    ).hasRole("ADMIN")

                    // 5. EXCLUSIVO DE ADMIN (Todo el resto de /api/admin/)
                    .requestMatchers(
                        "/api/admin/**",
                        "/auth/signup/admin"
                    ).hasRole("ADMIN")

                    // 6. USUARIOS REGULARES (Y staff)
                    .requestMatchers(
                        "/api/user/**",
                        "/api/cart/**",
                        "/api/orders/**",
