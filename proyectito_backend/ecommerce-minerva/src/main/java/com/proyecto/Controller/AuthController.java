        ...
        try {
            ...
            if (userRepository.findByEmail(request.getEmail()) != null) {
                ...
                return ResponseEntity
                    ...
                    .body(new ApiResponse<>("El email ya está en uso", false, null));
            }
            ...
            welcomeCoupon.setName("Cupón de Bienvenida");
            welcomeCoupon.setCode("BIENVENIDA-" + UUID.randomUUID().toString().substring(0, 6).toUpperCase());
            welcomeCoupon.setDiscount(40);
            welcomeCoupon.setType("percentage");
            welcomeCoupon.setStartDate(LocalDateTime.now());
            welcomeCoupon.setEndDate(LocalDateTime.now().plusDays(30)); // Válido por 30 días
            welcomeCoupon.setUsageLimit(1);
            welcomeCoupon.setUsageCount(0);
            welcomeCoupon.setStatus("activo");
            welcomeCoupon.setMinPurchase(0.0);
            welcomeCoupon.setUser(savedUser);
            ...
            return ResponseEntity.status(HttpStatus.CREATED)
            		.body(new ApiResponse<>("Registro exitoso", true, new AuthResponse(token,"Registro exitoso")));

        } catch (Exception e) {
            logger.error("Error durante el registro: {}", request.getEmail(), e);
            return ResponseEntity
                .status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body(new ApiResponse<>("Error durante el registro", false, null));
        }
    }
        ...
        try {
            ...
            if (user == null) {
                ...
                return ResponseEntity
                    ...
                    .body(new ApiResponse<>("Credenciales inválidas", false, null));
            }
            ...
            // Construir respuesta
            return ResponseEntity.status(HttpStatus.CREATED)
            		.body(new ApiResponse<>("Login Exitoso", true, new AuthResponse(token,"Login Exitoso")));

        } catch (Exception e) {
            logger.error("Error durante el login: {}", request.getEmail(), e);
            return ResponseEntity
                .status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body(new ApiResponse<>("Error durante el login", false, null));
        }
    }
