...
export class CartService {
  ...
  cartTotal = computed(() => {
    return this.cartItemsSignal().reduce((total, item) => {
      ...
      const price = p.precio !== undefined ? p.precio : (p.price !== undefined ? p.price : 0);
  ...
  cartGrandTotal = computed(() => {
    let sub = this.cartTotal();
    if (this.couponCode()) {
      sub = sub - (sub * (this.couponDiscount() / 100));
    }
    return sub + this.shipping();
  });
  ...
  addToCart(product: Product, quantity: number = 1, size: string, color: string): void {
    ...
    if (existingItemIndex > -1) {
      ...
      if (newQuantity > stockLimit) {
        // Limit to max stock instead of exceeding
      ...
      // Si no existe, agregar nuevo item respetando el stock
    ...
    try {
      ...
      if (cartData) {
        ...
        if (parsed && Array.isArray(parsed)) {
          ...
          this.cartItemsSignal.set(parsed.items);
          if (parsed.couponCode) {
             this.couponCode.set(parsed.couponCode);
             this.couponDiscount.set(parsed.couponDiscount || 0);
          }
        }
      }
    } catch (error) {
