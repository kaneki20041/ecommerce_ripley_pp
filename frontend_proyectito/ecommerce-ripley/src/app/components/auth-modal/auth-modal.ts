...
export class AuthModalComponent {
  ...
  constructor(
    ...
    // Resetear mensajes y limpiar formularios de forma síncrona e instantánea al abrir/cerrar
    this.authModalService.onOpenOrClose = () => {
      this.clearMessages();
      this.resetForms();
    };
  }
    ...
    this.authService
      ...
      .subscribe({
        ...
        error: (error) => {
          ...
          this.errorMessage.set(error.error?.message || 'Error al registrarse. Intenta nuevamente.');
        },
      });
  }
  ...
  onForgotPassword() {
    ...
    //     },
    //     error: (error) => {
    //       this.errorMessage.set(error.error?.message || 'Error al enviar correo. Intenta nuevamente.');
    //     },
    //   });
  }
