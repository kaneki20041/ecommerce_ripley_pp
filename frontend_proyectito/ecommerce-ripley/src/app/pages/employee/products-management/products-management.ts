...
import { ProductAdminListResponse, CreateProductRequest, UpdateProductBasicRequest, UpdateVariantRequest, Color, Talla } from '../../../models/product.model';
...
export class ProductsManagementComponent {
  ...
  categoriesTree: CategoryResponse[] = [];
  level1Categories: CategoryResponse[] = [];
  level2Categories: CategoryResponse[] = [];
  level3Categories: CategoryResponse[] = [];
    ...
    this.productService.getPaginatedAdminProducts(this.currentPage, this.pageSize)
      .subscribe({
        next: (response) => {
          ...
          // Mapear jerarquía de categoría
          const catId = Number(this.productForm.categoria);
          if (catId) {
            const path: CategoryResponse[] = [];
            this.findCategoryPath(catId, this.categoriesTree, path);
            if (path.length > 0) {
              this.selectedL1 = path[0].id.toString();
              this.level2Categories = (path[0].subcategories || []).filter(c => c.activo);
              if (path.length > 1) {
                this.selectedL2 = path[1].id.toString();
                this.level3Categories = (path[1].subcategories || []).filter(c => c.activo);
                if (path.length > 2) {
                  this.selectedL3 = path[2].id.toString();
                }
              }
            }
          }
  ...
  cargarDatosComplementarios(): void {
    // Obtenemos colores y tallas
    this.http.get<ApiResponse<Color[]>>(`${environment.apiUrl}/api/colors`).subscribe({
      next: (res) => this.availableColors = res.data,
      error: (err) => console.error('Error al cargar colores', err)
    });
    this.http.get<ApiResponse<Talla[]>>(`${environment.apiUrl}/api/tallas`).subscribe({
      next: (res) => this.availableTallas = res.data,
    ...
    this.categoryService.getAdminCategoryTree().subscribe({
      next: (res) => {
        if (res.result && res.data) {
          ...
          this.level1Categories = this.categoriesTree.filter(c => c.activo);
        }
      },
  ...
  getTallasPorCategoria(): Talla[] {
    const catId = Number(this.productForm.categoria);
    let catName = '';
    let isCalzado = false;
    
    const path: CategoryResponse[] = [];
    if (this.findCategoryPath(catId, this.categoriesTree, path) && path.length > 0) {
      catName = path[path.length - 1].name.toLowerCase();
      const parentName = path.length > 1 ? path[path.length - 2].name.toLowerCase() : '';
      isCalzado = catName === 'calzado' || parentName === 'calzado' || catName === 'zapatos' || catName.includes('calzado') || catName.includes('zapatos');
    }
    
    return this.availableTallas.filter(t => isCalzado ? t.tipo === 'Calzado' : t.tipo === 'General' || t.tipo === 'Ropa');
  }
  ...
  openEditModal(row: ProductAdminListResponse): void {
    ...
    this.productService.getVariantByIdByAdmin(this.currentVariantId).subscribe({
      next: (res) => {
        if (res.result && res.data) {
          ...
          this.productForm = {
            ...
            categoria: v.categoriaId || row.categoriaId || '',
            genero: v.genero || '',
            marca: v.marca || '',
            material: v.material || '',
            colores: [v.color],
          ...
          this.productForm.colorDetails[v.color] = {
            ...
            precioAnterior: v.discountedPrice || 0,
      ...
      error: (err) => {
        ...
        this.toastService.error('Error', 'Error de red al cargar el producto para edición');
        this.loading.set(false);
      }
    });
  }
  ...
  onLevel1Change(): void {
    ...
    if (this.selectedL1) {
      const parent = this.level1Categories.find(c => c.id.toString() === this.selectedL1);
      if (parent) {
        this.level2Categories = (parent.subcategories || []).filter(c => c.activo);
        this.autoMapGender(parent.name);
      }
    }
  ...
  onLevel2Change(): void {
    ...
    this.level3Categories = [];
    if (this.selectedL2) {
      const parent = this.level2Categories.find(c => c.id.toString() === this.selectedL2);
      if (parent) {
        this.level3Categories = (parent.subcategories || []).filter(c => c.activo);
      }
    }
    this.updateFinalCategory();
  }
  ...
  updateFinalCategory(): void {
    if (this.selectedL3) {
      this.productForm.categoria = this.selectedL3;
    } else if (this.selectedL2) {
      this.productForm.categoria = this.selectedL2;
    } else if (this.selectedL1) {
      this.productForm.categoria = this.selectedL1;
    } else {
      this.productForm.categoria = '';
    }
  }

  autoMapGender(catName: string): void {
    const nameUpper = catName.toUpperCase();
    if (nameUpper.includes('HOMBRE') || nameUpper.includes('NIÑO')) {
      this.productForm.genero = 'Hombre';
    } else if (nameUpper.includes('MUJER') || nameUpper.includes('NIÑA')) {
      this.productForm.genero = 'Mujer';
    } else {
      this.productForm.genero = 'Unisex';
    }
  }

  findCategoryPath(targetId: number, nodes: CategoryResponse[], path: CategoryResponse[]): boolean {
    for (const node of nodes) {
      path.push(node);
      if (node.id === targetId) return true;
      if (node.subcategories && node.subcategories.length > 0) {
        if (this.findCategoryPath(targetId, node.subcategories, path)) return true;
      }
      path.pop();
    }
    return false;
  ...
  private buildProductRequest(): CreateProductRequest {
    ...
    if (colors.length > 0) {
      ...
      colors.forEach(col => {
        ...
        coloresDataPayload.push({
          ...
          price: det.precio || 0,
          discountedPrice: det.precioAnterior || 0,
    ...
    return {
      ...
      categoriaId: Number(this.productForm.categoria),
  ...
  createProduct(): void {
    ...
    this.productService.createProductByAdmin(req).subscribe({
      next: (res) => {
        if (res.result) {
          this.toastService.success('Éxito', 'Producto y variantes creados correctamente');
          this.closeModal();
          this.resetForm();
      ...
      error: (err) => {
        ...
        this.toastService.error('Error', 'Error de conexión al crear producto');
        this.loading.set(false);
      }
    });
  }
  ...
  updateProduct(): void {
    if (this.productForm.colores.length === 0) {
      this.toastService.warning('Advertencia', 'Debe existir color en la variante.');
      return;
    }
    ...
    const variantReq: UpdateVariantRequest = {
      ...
      discountedPrice: details.precioAnterior || 0,
    ...
    forkJoin([updateVariant$, updateBasicInfo$]).subscribe({
      next: ([variantRes, basicInfoRes]) => {
        ...
        if (variantRes.result && basicInfoRes.result) {
          ...
          this.resetForm();
          this.cargarProductosDesdeServidor();
          this.toastService.success('Éxito', 'Producto y variante actualizados correctamente.');
        } else {
          ...
          this.toastService.error('Error', 'Error al actualizar: ' + errorMsg);
          this.loading.set(false);
        }
      },
      error: (err) => {
        console.error('Error durante la actualización múltiple:', err);
        this.toastService.error('Error', 'Error de conexión al guardar los cambios.');
        this.loading.set(false);
      }
    });
  }
    ...
    this.productService.toggleVariantStatusByAdmin(targetId).subscribe({
      next: (res: any) => {
        if (res.result) {
          ...
          this.toastService.warning('Advertencia', 'No se pudo desactivar: ' + res.message);
        }
        this.loading.set(false);
      },
      error: (err: any) => {
        console.error('Error al desactivar:', err);
        this.toastService.error('Error', 'Error de red al desactivar');
        this.loading.set(false);
      }
    });
    ...
    this.productService.toggleVariantStatusByAdmin(targetId).subscribe({
      next: (res: any) => {
        if (res.result) {
          ...
          this.toastService.warning('Advertencia', 'No se pudo activar: ' + res.message);
        }
        this.loading.set(false);
      },
      error: (err: any) => {
        console.error('Error al activar:', err);
        this.toastService.error('Error', 'Error de red al activar');
        this.loading.set(false);
      }
    });
